...
import { Agent, ChatCompletionMessage, AgentFunction, Response, Result } from "./types";
...
export class Swarm {
    ...
    async getChatCompletion(
        ...
        const instructions = typeof agent.instructions === "function" ? agent.instructions(contextVars) : agent.instructions;
    ...
    handleFunctionResult(result: any, debug: boolean): Result {
        if (result instanceof Result) {
            ...
            try {
                ...
                const errorMsg = `Failed to cast response to string: ${result}. Error: ${(e as Error).message}`;
